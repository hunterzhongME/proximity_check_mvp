// generated from rosidl_generator_py/resource/_idl_support.c.em
// with input from melfa_msgs:msg/GpioState.idl
// generated code does not contain a copyright notice
#define NPY_NO_DEPRECATED_API NPY_1_7_API_VERSION
#include <Python.h>
#include <stdbool.h>
#ifndef _WIN32
# pragma GCC diagnostic push
# pragma GCC diagnostic ignored "-Wunused-function"
#endif
#include "numpy/ndarrayobject.h"
#ifndef _WIN32
# pragma GCC diagnostic pop
#endif
#include "rosidl_runtime_c/visibility_control.h"
#include "melfa_msgs/msg/detail/gpio_state__struct.h"
#include "melfa_msgs/msg/detail/gpio_state__functions.h"

#include "rosidl_runtime_c/string.h"
#include "rosidl_runtime_c/string_functions.h"


ROSIDL_GENERATOR_C_EXPORT
bool melfa_msgs__msg__gpio_state__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    char full_classname_dest[37];
    {
      char * class_name = NULL;
      char * module_name = NULL;
      {
        PyObject * class_attr = PyObject_GetAttrString(_pymsg, "__class__");
        if (class_attr) {
          PyObject * name_attr = PyObject_GetAttrString(class_attr, "__name__");
          if (name_attr) {
            class_name = (char *)PyUnicode_1BYTE_DATA(name_attr);
            Py_DECREF(name_attr);
          }
          PyObject * module_attr = PyObject_GetAttrString(class_attr, "__module__");
          if (module_attr) {
            module_name = (char *)PyUnicode_1BYTE_DATA(module_attr);
            Py_DECREF(module_attr);
          }
          Py_DECREF(class_attr);
        }
      }
      if (!class_name || !module_name) {
        return false;
      }
      snprintf(full_classname_dest, sizeof(full_classname_dest), "%s.%s", module_name, class_name);
    }
    assert(strncmp("melfa_msgs.msg._gpio_state.GpioState", full_classname_dest, 36) == 0);
  }
  melfa_msgs__msg__GpioState * ros_message = _ros_message;
  {  // interface_name
    PyObject * field = PyObject_GetAttrString(_pymsg, "interface_name");
    if (!field) {
      return false;
    }
    assert(PyUnicode_Check(field));
    PyObject * encoded_field = PyUnicode_AsUTF8String(field);
    if (!encoded_field) {
      Py_DECREF(field);
      return false;
    }
    rosidl_runtime_c__String__assign(&ros_message->interface_name, PyBytes_AS_STRING(encoded_field));
    Py_DECREF(encoded_field);
    Py_DECREF(field);
  }
  {  // bitid
    PyObject * field = PyObject_GetAttrString(_pymsg, "bitid");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->bitid = (uint16_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
  {  // bitmask
    PyObject * field = PyObject_GetAttrString(_pymsg, "bitmask");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->bitmask = (uint16_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
  {  // bit_send_type
    PyObject * field = PyObject_GetAttrString(_pymsg, "bit_send_type");
    if (!field) {
      return false;
    }
    assert(PyUnicode_Check(field));
    PyObject * encoded_field = PyUnicode_AsUTF8String(field);
    if (!encoded_field) {
      Py_DECREF(field);
      return false;
    }
    rosidl_runtime_c__String__assign(&ros_message->bit_send_type, PyBytes_AS_STRING(encoded_field));
    Py_DECREF(encoded_field);
    Py_DECREF(field);
  }
  {  // input_data
    PyObject * field = PyObject_GetAttrString(_pymsg, "input_data");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->input_data = (uint16_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }
  {  // output_data
    PyObject * field = PyObject_GetAttrString(_pymsg, "output_data");
    if (!field) {
      return false;
    }
    assert(PyLong_Check(field));
    ros_message->output_data = (uint16_t)PyLong_AsUnsignedLong(field);
    Py_DECREF(field);
  }

  return true;
}

ROSIDL_GENERATOR_C_EXPORT
PyObject * melfa_msgs__msg__gpio_state__convert_to_py(void * raw_ros_message)
{
  /* NOTE(esteve): Call constructor of GpioState */
  PyObject * _pymessage = NULL;
  {
    PyObject * pymessage_module = PyImport_ImportModule("melfa_msgs.msg._gpio_state");
    assert(pymessage_module);
    PyObject * pymessage_class = PyObject_GetAttrString(pymessage_module, "GpioState");
    assert(pymessage_class);
    Py_DECREF(pymessage_module);
    _pymessage = PyObject_CallObject(pymessage_class, NULL);
    Py_DECREF(pymessage_class);
    if (!_pymessage) {
      return NULL;
    }
  }
  melfa_msgs__msg__GpioState * ros_message = (melfa_msgs__msg__GpioState *)raw_ros_message;
  {  // interface_name
    PyObject * field = NULL;
    field = PyUnicode_DecodeUTF8(
      ros_message->interface_name.data,
      strlen(ros_message->interface_name.data),
      "replace");
    if (!field) {
      return NULL;
    }
    {
      int rc = PyObject_SetAttrString(_pymessage, "interface_name", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }
  {  // bitid
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->bitid);
    {
      int rc = PyObject_SetAttrString(_pymessage, "bitid", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }
  {  // bitmask
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->bitmask);
    {
      int rc = PyObject_SetAttrString(_pymessage, "bitmask", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }
  {  // bit_send_type
    PyObject * field = NULL;
    field = PyUnicode_DecodeUTF8(
      ros_message->bit_send_type.data,
      strlen(ros_message->bit_send_type.data),
      "replace");
    if (!field) {
      return NULL;
    }
    {
      int rc = PyObject_SetAttrString(_pymessage, "bit_send_type", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }
  {  // input_data
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->input_data);
    {
      int rc = PyObject_SetAttrString(_pymessage, "input_data", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }
  {  // output_data
    PyObject * field = NULL;
    field = PyLong_FromUnsignedLong(ros_message->output_data);
    {
      int rc = PyObject_SetAttrString(_pymessage, "output_data", field);
      Py_DECREF(field);
      if (rc) {
        return NULL;
      }
    }
  }

  // ownership of _pymessage is transferred to the caller
  return _pymessage;
}
